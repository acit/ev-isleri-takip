...
const initTables = async () => {
  ...
  if (!existingUser) {
    ...
    // Create test user
    await db.run(
      'INSERT INTO users (email, full_name, family_id, role, status) VALUES (?, ?, ?, ?, ?)',
      ['test@example.com', 'Test Kullanıcı', familyResult.lastID, 'admin', 'active']
    );
    ...
    for (const task of sampleTasks) {
      await db.run(
        'INSERT INTO tasks (family_id, title, description, created_by, frequency, due_date, priority, category, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [familyResult.lastID, task.title, task.description, 1, task.frequency, task.due_date, task.priority, task.category, 'pending']
      );
    }
    ...
    for (const item of sampleInventory) {
      await db.run(
        'INSERT INTO inventory (family_id, item_name, quantity, unit, min_threshold, category, location, last_updated) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
        [familyResult.lastID, item.item_name, item.quantity, item.unit, item.min_threshold, item.category, item.location, 1]
      );
    }
    ...
    for (const budget of sampleBudgets) {
      await db.run(
        'INSERT INTO budgets (family_id, category, monthly_limit, spent_amount, month_year) VALUES (?, ?, ?, ?, ?)',
        [familyResult.lastID, budget.category, budget.monthly_limit, budget.spent_amount, currentMonth]
      );
    }
    ...
    for (const expense of sampleExpenses) {
      await db.run(
        'INSERT INTO expenses (family_id, recorded_by, amount, category, description, expense_date) VALUES (?, ?, ?, ?, ?, ?)',
        [familyResult.lastID, 1, expense.amount, expense.category, expense.description, expense.expense_date]
      );
    }
    ...
    for (const msg of sampleMessages) {
      await db.run(
        'INSERT INTO messages (family_id, sender_id, message, is_group_message, created_at) VALUES (?, ?, ?, ?, ?)',
        [familyResult.lastID, 1, msg.message, msg.is_group_message, msg.created_at]
      );
    }
