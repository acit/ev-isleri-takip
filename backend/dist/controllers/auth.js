...
exports.getEmailStatus = exports.acceptInvite = exports.inviteFamilyMember = exports.verifyLoginCode = exports.sendLoginCode = exports.confirmPasswordReset = exports.resetPassword = exports.setPassword = exports.loginWithPassword = void 0;
...
const loginWithPassword = async (req, res) => {
    try {
        ...
        const user = await db.get('SELECT id, email, password_hash, family_id, role, full_name, status FROM users WHERE email = ?', [email]);
        if (!user) {
...
const setPassword = async (req, res) => {
    try {
        ...
        // Update password
        await db.run('UPDATE users SET password_hash = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [hashedPassword, userId]);
...
const resetPassword = async (req, res) => {
    try {
        ...
        // Store reset code
        await db.run('INSERT OR REPLACE INTO login_codes (email, code, expires_at) VALUES (?, ?, ?)', [email, resetCode, expiresAt.toISOString()]);
        // Send reset email
        await (0, email_1.sendEmail)(email, 'Aile Takip Sistemi - Şifre Sıfırlama', `
...
const confirmPasswordReset = async (req, res) => {
    try {
        ...
        const db = await (0, database_1.getDatabase)();
        // Verify reset code
        const resetCode = await db.get('SELECT * FROM login_codes WHERE email = ? AND code = ? AND expires_at > datetime("now")', [email, code]);
        if (!resetCode) {
        ...
        const hashedPassword = await bcryptjs_1.default.hash(newPassword, saltRounds);
        // Update password
        const result = await db.run('UPDATE users SET password_hash = ?, updated_at = CURRENT_TIMESTAMP WHERE email = ?', [hashedPassword, email]);
        if (result.changes === 0) {
            return res.status(404).json({ error: 'Kullanıcı bulunamadı' });
        }
        // Delete used code
...
const sendLoginCode = async (req, res) => {
    try {
        ...
        if (!user) {
            ...
            const result = await db.run('INSERT INTO users (email, full_name, status) VALUES (?, ?, ?)', [email, email.split('@')[0], 'pending']);
        ...
        await db.run('DELETE FROM login_codes WHERE email = ?', [email]);
        await db.run('INSERT INTO login_codes (email, code, expires_at) VALUES (?, ?, ?)', [email, code, expiresAt.toISOString()]);
    ...
    `;
        await (0, email_1.sendEmail)(email, emailSubject, emailContent);
        res.json({
            message: isNewUser ?
...
const verifyLoginCode = async (req, res) => {
    try {
        ...
        const db = await (0, database_1.getDatabase)();
        // Verify code
        const loginCode = await db.get('SELECT * FROM login_codes WHERE email = ? AND code = ? AND expires_at > datetime("now")', [email, code]);
        if (!loginCode) {
            return res.status(400).json({ error: 'Geçersiz veya süresi dolmuş kod' });
        }
        // Get user
        const user = await db.get('SELECT id, family_id, role, full_name FROM users WHERE email = ?', [email]);
        // If user doesn't have a family, create one
        let familyId = user.family_id;
        if (!familyId) {
            const familyName = `${user.full_name || email.split('@')[0]} Ailesi`;
            const familyResult = await db.run('INSERT INTO families (name, created_by) VALUES (?, ?)', [familyName, user.id]);
            familyId = familyResult.lastID;
            // Update user with family_id and set as admin
            await db.run('UPDATE users SET family_id = ?, role = ?, status = ? WHERE id = ?', [familyId, 'admin', 'active', user.id]);
...
const inviteFamilyMember = async (req, res) => {
    try {
        ...
        // Create new user with pending status
        await db.run('INSERT INTO users (email, family_id, status) VALUES (?, ?, ?)', [email, familyId, 'pending']);
...
const acceptInvite = async (req, res) => {
    try {
        ...
        const result = await db.run('UPDATE users SET status = ? WHERE email = ?', ['active', email]);
        if (result.changes === 0) {
...
const getEmailStatus = async (req, res) => {
    try {
        const status = (0, emailHelper_1.getEmailStatus)();
        res.json(status);
    }
    catch (error) {
