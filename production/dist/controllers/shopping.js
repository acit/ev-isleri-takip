...
exports.sendShoppingListByEmail = exports.updateShoppingListItem = exports.getShoppingListItems = exports.generateShoppingListFromLowStock = exports.getShoppingLists = exports.createShoppingList = void 0;
...
const createShoppingList = async (req, res) => {
    try {
        ...
        const listResult = await db.run('INSERT INTO shopping_lists (family_id, created_by, title, status) VALUES (?, ?, ?, ?)', [req.familyId, req.userId, title || 'Alışveriş Listesi', 'draft']);
        // Add items if provided
        if (items && items.length > 0) {
            for (const item of items) {
                await db.run('INSERT INTO shopping_list_items (shopping_list_id, item_name, quantity, unit, estimated_cost) VALUES (?, ?, ?, ?, ?)', [listResult.lastID, item.item_name, item.quantity, item.unit, item.estimated_cost]);
            }
        }
...
const getShoppingLists = async (req, res) => {
    try {
        const db = await (0, database_1.getDatabase)();
        const lists = await db.all('SELECT * FROM shopping_lists WHERE family_id = ? ORDER BY created_at DESC', [req.familyId]);
        res.json(lists);
    }
    catch (error) {
...
const generateShoppingListFromLowStock = async (req, res) => {
    try {
        const db = await (0, database_1.getDatabase)();
        // Get low stock items
        const lowStockItems = await db.all('SELECT * FROM inventory WHERE family_id = ? AND quantity <= min_threshold', [req.familyId]);
        if (lowStockItems.length === 0) {
            return res.json({ message: 'No low stock items found' });
        }
        // Create shopping list
        const listResult = await db.run('INSERT INTO shopping_lists (family_id, created_by, title, auto_generated, status) VALUES (?, ?, ?, ?, ?)', [req.familyId, req.userId, 'Otomatik Alışveriş Listesi', true, 'draft']);
        // Add low stock items to shopping list
        for (const item of lowStockItems) {
            const neededQuantity = Math.max(item.min_threshold * 2 - item.quantity, 1);
            await db.run('INSERT INTO shopping_list_items (shopping_list_id, item_name, quantity, unit) VALUES (?, ?, ?, ?)', [listResult.lastID, item.item_name, neededQuantity, item.unit]);
        }
        const newList = await db.get('SELECT * FROM shopping_lists WHERE id = ?', [listResult.lastID]);
        res.json(newList);
    }
...
exports.generateShoppingListFromLowStock = generateShoppingListFromLowStock;
const getShoppingListItems = async (req, res) => {
    try {
        ...
        // Get items
        const items = await db.all('SELECT * FROM shopping_list_items WHERE shopping_list_id = ? ORDER BY id', [listId]);
        res.json({ list, items });
    }
    catch (error) {
...
const updateShoppingListItem = async (req, res) => {
    ...
    `, [itemId, req.familyId]);
        ...
        values.push(itemId);
        await db.run(`UPDATE shopping_list_items SET ${updates.join(', ')} WHERE id = ?`, values);
        // Get updated item
...
const sendShoppingListByEmail = async (req, res) => {
    try {
        const { listId } = req.params;
    ...
    `;
        ...
        for (const email of recipientEmails) {
            await sendEmail(email.trim(), `🛒 Alışveriş Listesi: ${list.title}`, emailHtml);
        }
        // Update list as sent
        await db.run('UPDATE shopping_lists SET sent_via_email = 1, sent_at = CURRENT_TIMESTAMP, status = ? WHERE id = ?', ['sent', listId]);
...
exports.sendShoppingListByEmail = sendShoppingListByEmail;
